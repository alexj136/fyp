...
\\\indent There is a single constructor for values in the product type, which takes two values and returns those values in a tuple. The syntax for this is \texttt{\{ E , F \}}, where E and F are expressions. There are two destructors for values in the product type, which are \texttt{fst} and \texttt{snd}, which retrieve the first and second values in a tuple, respectively.
