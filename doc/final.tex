...
\noindent\textbf{REVISE}\\
One difficulty in implementing thi\\
To avoid this, we first reduce the application of the function one step, which removes the constraint that our function has the type ..., whilst still ensuring that it is used only in type-safe ways. However this causes the problem that functions that are never used may contain type errors, because we never check them.
\\\textbf{END-REVISE}\\
