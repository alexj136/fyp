...
\\\\
We might call this function using:
\\\\
...
\[ \lambda x.x * x \]
Immediately we see that the function name, \texttt{square}, is no longer present. This is because functions in the $\lambda$ calculus do not have names. This means that in order to use the function, we must write it down in its entirety, rather than refer to it by name like we did in the earlier example.
\\ % THIS IS WHERE I AM
The syntax of the $\lambda$ calculus is given by the context-free grammar:
\[ M ::= x | \lambda x.M | MN \]
